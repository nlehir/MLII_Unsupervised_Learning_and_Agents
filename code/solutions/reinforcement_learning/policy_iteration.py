...
def plot_value_function(value_function, loop):
    title = "value function at loop {}".format(loop)
    plt.imshow(value_function)
    plt.colorbar()
    plt.title(title)
    plt.savefig(
        "images/policy_iteration/value_function_loop_{}.pdf".format(loop))
    plt.close()
...
def improve_policy(policy, value_function):
    for state in policy:
        ...
        neghbor_values = {left: value_function[left],
                          top: value_function[top],
                          right: value_function[right],
                          bottom: value_function[bottom]}
        sorted_values = \
            sorted(neghbor_values,
                   key=lambda action: neghbor_values[action], reverse=True)
